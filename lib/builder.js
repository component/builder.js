...
Builder.prototype.buildType = function(type, fn, process){
  ...
  this.json(function(err, conf){
    ...
    // "before <type>" hook
    self.hook('before ' + type, self, function(error) {
      if (error) done(error);
...
Builder.prototype.hook = function(name, fn){
  ...
  return this.performHook.apply(this, arguments);
};
...
Builder.prototype.performHook = function(name, arg, callback){
  debug('invoke hook "%s"', name);
  var fns = this._hooks[name] || [];
  var fn = null;
  var lastError = null;
  var completed = 0;
  var increaseCompleted = function(error) {
    completed++;
  ...
  for (var i = 0, len = fns.length; i < len; ++i) {
    ...
    // Check arity as the hook can be async or sync.
    if (fn.length === 1) {
      fn(arg);
      increaseCompleted();
    } else {
      fn(arg, increaseCompleted);
    }
  }
