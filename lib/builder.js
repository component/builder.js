...
function Builder(dir, parent) {
  ...
  this.on('dependency', function(that){
    that.lookups = self.lookups;
  });
}
...
/**
 * Set up default lookup paths for this
 * and any subsequent dependencies by
 * specifying a string or array for `locations`
 *
 * @param {String|Array} locations
 * @api public
 */

Builder.prototype.setLookups = function(locations){
  this.lookups = [];
  if ('string' == typeof locations) {
    this.lookups.push(locations);
  } else {
