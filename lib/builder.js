...
/**
 * Enable separate files in the build.
 *
 * @api public
 */

Builder.prototype.separate = function(){
  debug('separate files enabled');
  this.sep = true;
};
...
Builder.prototype.build = function(fn){
  ...
  batch.end(function(err, res){
    if (err) return fn(err);
    var js = self.sep ?
      res.shift().concat({file: 'require-alias.js', content: res.shift()}) :
      res.shift() + '\n' + res.shift();
    fn(null, {
      js: js,
      css: res.shift(),
...
Builder.prototype.buildType = function(type, fn, process){
  ...
  this.json(function(err, conf){
    ...
    if (conf[type]) {
      conf[type].forEach(function(file){
        ...
        batch.push(function(done){
          fs.readFile(path, 'utf8', function(err, str){
            if (err) return fn(err);
            var content = process(self, file, str);
            if (self.sep) {
              content = {
                file: normalize(self.name + '/' + file),
                content: content
              };
            }
            done(null, content);
    ...
    batch.end(function(err, res){
      if (err) return fn(err);
      fn(null, self.sep ? res.reduce(function (a, b) { return a.concat(b); }, []) : res.join('\n'));
    });
  });
};
