...
Builder.prototype.inherit = function(dep){
  ...
  this.scriptTypes.forEach(function(type){dep.addScriptType(type);});
...
Builder.prototype.addScriptType = function(type){
  this.scriptTypes.pop(type);
              ...
              self.scriptTypes.forEach(function(type){
                ...
                if (scripts){
                  var typeAliases = scripts.map(function(script){
                    if (type != 'scripts'){
                      script = path.basename(script, '.'+type) + '.js';
                    }
                    var alias = self.root
